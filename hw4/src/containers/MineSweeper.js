...
const MineSweeper = () => {
    ...
    const mineNumOnChange = (/*value?*/) => {
        let mine_num_value = document.getElementById('MinesNumber').value;
        setMineNum(mine_num_value);
    }

    // Advanced TODO: Change `boardSize` to the number you send by this function
    const boardSizeOnChange = (/*value?*/) => {
        let board_size_value = document.getElementById('BoardSize').value;
        setBoardSize(board_size_value);
    }
